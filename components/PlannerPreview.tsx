...
const PlannerPreview: React.FC<PlannerPreviewProps> = ({ plan, onBack }) => {
  ...
  useEffect(() => {
    // This timeout gives the browser a moment to render all the generated pages
    // before enabling the print button. For very large plans, this is crucial.
    const timer = setTimeout(() => {
      setIsLoading(false);
    }, 1500); // 1.5 second delay to ensure rendering completes
  ...
  return (
    <div className="bg-slate-200">
      <div className="fixed top-0 left-0 right-0 z-50 flex justify-between p-4 bg-white/80 backdrop-blur-sm shadow-sm no-print no-print-header">
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-white/70 flex items-center justify-center z-40 no-print">
          <div className="text-center p-8 bg-white rounded-lg shadow-xl">
            ...
            <p className="text-slate-500 mt-2">Please wait while we prepare your {totalDays}-page document.</p>
      ...
      <div className={`mx-auto flex flex-col items-center gap-8 py-8 pt-24 transition-opacity duration-500 ${isLoading ? 'opacity-0' : 'opacity-100'}`}>
        {/* Title Page */}
